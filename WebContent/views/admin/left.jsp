...
<html>
  <head>
    <title>My JSP 'left.jsp' starting page</title>
		...
		<link rel="stylesheet" href="<c:url value='/views/css/admin/style.css'/>" />
    	<script type="text/javascript" src="<c:url value="/views/js/admin/jquery.min.js"/>"></script>
    	<script type="text/javascript" src="<c:url value="/views/js/admin/main.js"/>"></script>
  </head>
<script>
  function dianji(id){
	var src =$("."+id).attr("src");
					...
					<c:choose>
						...
						<c:otherwise>
							<c:forEach items="${parent.children}" var="child">
								<li>
									<a class="lian" href="/cmj/admin/adminBook?cid=${child.cid}" target="body">${child.cname}</a>
								</li>
							</c:forEach>
						</c:otherwise>
					</c:choose>
