...
<html>
  <head>
    <title>订单详细</title>
		...
		<link rel="stylesheet" type="text/css" href="<c:url value='/views/css/backorder/desc.css'/>">
  </head>
...
<c:forEach items="${order.orderItems }" var="orderItem">
						<tr style="padding-top: 20px; padding-bottom: 20px;">
							<td class="td" width="400px">
								<div class="bookname">
								   <a href="<c:url value='/admin/adminLoadBook?bid=${orderItem.book.bid }'/>">
								   <img align="middle" width="70" src="<c:url value='/${orderItem.book.imageb }'/>"/></a>
								  <a href="<c:url value='/admin/adminLoadBook?bid=${orderItem.book.bid }'/>">${orderItem.book.bname }</a>
								</div>
							</td>
							<td class="td" >
...
<c:if test="${order.status eq 1 and btn eq 'cancel' }">
	<a title="取消订单" id="cancel" href="<c:url value='/admin/AdminOrderServlet?method=cancel&oid=${order.oid }'/>">取　　消</a><br>
		...
		<br>
		<a title="返回列表" style="font-size:1.2em" class="button11" href="<c:url value='/admin/adminfindallorder'/>" target="body">返回订单管理</a>
		<br>
		</div>
	</div>
</body>
