...
<link rel="stylesheet" href="<c:url value='/views/css/backcategory/category.css'/>" />
...
if($(".categorymsg").text().length>0){
	alert($(".categorymsg").text());
}
});
function dianji(id){
...
<div class="am-cf admin-main">
	...
	<div class="fenlei" style="display:none">
		<form action="<c:url value='/admin/addCategory'/>" id="yijiform" method="get">
			分类名称:<input type="text" name="cname" autocomplete="off" class="yijifenlei"/>
	...
	<div class="admin-biaogelist">
		<div class="listbiaoti am-cf">
			...
			<dl class="am-icon-home" style="float: right;">当前位置： 首页 > <a href="#">商品栏目列表</a>
			</dl>
			<dl>
				<button type="button" id="fenlei" class="am-btn am-btn-danger am-round am-btn-xs am-icon-plus">添加商品一级分类</button>
			</dl>
			<dl>
				<span class="categorymsg" style="display: none;">${categorymsg}</span>
			</dl>
		</div>
	...
	<div class="cd-accordion-menu">
		<c:forEach items="${parents}" var="parent" varStatus="status">
			...
			<a title="删除分类" style="height: 35px;line-height: 31px;font-size: 15px;font-weight: 500;"
				...
				href="<c:url value='/admin/deleteParent?cid=${parent.cid }'/>">删除</a>
					</div>
		<label onclick="dianji(this.id)" id="i${status.index}"
		style="width:50%;float: right; font-size: 25px;" for="labelfup${status.index}">
			<div class="fu">
				<span style="margin-top: -4px;font-size: 30px;float: left;" class="ilei${status.index}">></span>
				<form id="2fup${status.index}form" action="<c:url value='/admin/editCategory'/>" method="post">
					...
					<input value="${parent.cname }" class="fucname2" autocomplete="off"
						...
						type="text" name="cname" id="32fup${status.index}input">
				</form>
				...
				<c:otherwise>
					<c:forEach items="${parent.children}" var="child">
						<li class="zi">
							...
							<form id="32c${child.cid}form" action="<c:url value='/admin/editCategory'/>" method="post">
									<input type="hidden" name="cid" value="${child.cid }"/>
									<input type="hidden" name="pid" value="${parent.cid }"/>
						<input value="${child.cname}" autocomplete="off" style="margin-left:-1px;display:none;position: absolute;width: 200px;height: 30px;margin-top:5px;"
						type="text" name="cname" id="432c${child.cid}">
						</form>
							</div>
							<div class="fu">
				<input title="保存"
				...
				<input title="取消"
					...
					id="2c${child.cid}" onclick="xiaobutton2(this.id)" class="button2" type="button" value="取消"> 
								...
								<a title="删除分类" style="height: 35px;line-height: 31px;font-size: 15px;font-weight: 500;"
									...
									href="<c:url value='/admin/deleteChild?cid=${child.cid }'/>">删除</a>
							</div></li>
		<script type="text/javascript">
		function jiaer(id) {
			var zhi = $(".ming"+id).text();
			 $(".zifen"+id).append("<li id='jiaer"+id+"' class='zi'><div class='fu' style='margin-left: 10%;width:40%;'>"+
					 "<form id='former"+id+"' action='<c:url value='/admin/addCategory'/>' method='post'>"+
					 ...
					 "type='text' id='cnameer"+id+"' name='cname'>"+
					 "</form></div>"+
