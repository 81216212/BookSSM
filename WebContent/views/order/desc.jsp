...
<!-- Cart -->
				<div class="woocommerce-cart container-fluid no-left-padding no-right-padding">
					...
					<div class="container">
							...
							<table class="table table-bordered table-responsive">
								...
								<tr>
									...
									<c:choose>
											...
											<c:when test="${order.status eq 5 }"><span style="color:red;">(已取消)</span></c:when>
									</c:choose>	
						...
						<tr>
							<td align="left">
								  <img align="middle" width="70" src="<c:url value='/${item.book.imageb }'/>"/>
								  <a style="text-decoration:none;" href="<c:url value='/book/selectByBid?bid=${item.book.bid }'/>">${item.book.bname }</a>
							</td>
							<td>
...
<c:if test="${order.status eq 1 }">
	<a href="<c:url value='/order/pay?oid=${order.oid }'/>" id="pay"></a><br/>
</c:if>
<c:if test="${order.status eq 1 and btn eq 'cancel'}">
    <a onclick="return confirm('您是否要取消该订单信息?')" id="cancel" href="<c:url value='/order/cancel?oid=${order.oid }'/>">取消订单</a><br/>
</c:if>
